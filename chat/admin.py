...
class ChatAdminForm(forms.ModelForm):
    ...
    conversation_history_text = forms.CharField(
        widget=forms.Textarea(attrs={
            'rows': 25,
            'cols': 100,
            'style': 'font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; width: 100%; padding: 10px;',
            'class': 'vLargeTextField',
            'placeholder': 'User: Your message here\nAssistant: AI response here\n\nUser: Another message\nAssistant: Another response'
        }),
        required=False,
        label='Conversation History',
        help_text='Enter conversation in readable format. Each message on a new line starting with "User:" or "Assistant:"'
    )
...
class ChatAdmin(admin.ModelAdmin):
    form = ChatAdminForm
    list_display = ('session_id', 'user', 'title', 'created_at', 'updated_at', 'conversation_length')
    ...
    fieldsets = (
        ...
        ('Conversation History', {
            ...
            'classes': ('wide',),
            'description': 'Edit conversation in readable format. Start each message with "User:" or "Assistant:" followed by the message content.'
        }),
        ('Timestamps', {
